...
@Suite struct TextRenderingTests {
    ...
    /// Writes a visual sheet when OWL_TEXT_SHEET is set; not an assertion, just for eyeballing.
    @Test func visualSheet() throws {
        guard let path = ProcessInfo.processInfo.environment["OWL_TEXT_SHEET"] else { return }
