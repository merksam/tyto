...
final class ScreenCapturer {
    ...
    /// Used for snap-to-window. Tyto's own windows are excluded by PID.
    ...
    func capture(displays: [DisplayInfo]) async throws -> [DisplaySnapshot] {
        guard !displays.isEmpty else { throw TytoError.noDisplays }
    ...
    /// Test harness: PNG bytes of one display as it currently looks, including Tyto's own windows.
    ...
    nonisolated static func captureOne(_ d: DisplayInfo) async throws -> DisplaySnapshot {
        ...
        guard let image = output.sdrImage else { throw TytoError.captureReturnedNoImage }
