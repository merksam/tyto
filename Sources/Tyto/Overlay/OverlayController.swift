...
final class OverlayController: SelectionViewDelegate, ToolbarDelegate {
    ...
    func beginSession(options: SessionOptions) async throws {
        guard session == nil else { throw TytoError.sessionAlreadyActive }
        ...
        guard Permissions.hasScreenCapture else { throw TytoError.noScreenCapturePermission }
        let all = capturer.displays()
        let wanted = options.displayIDs.map { ids in all.filter { ids.contains($0.id) } } ?? all
        guard !wanted.isEmpty else { throw TytoError.noDisplays }
    ...
    func copy() throws -> (rect: PixelRect, image: CGImage) {
        ...
        guard let id = session?.activeDisplay else { throw TytoError.noSession }
        let start = ContinuousClock.now
    ...
    /// Cmd+S: render the composite, dismiss the overlay, and present a save panel over ~/Pictures/Tyto.
    func save() {
        guard let s = session else { return }
    ...
    func setSelection(_ rect: PixelRect, on displayID: CGDirectDisplayID?) throws {
        guard var s = session else { throw TytoError.noSession }
        guard let id = displayID ?? s.activeDisplay ?? s.order.first,
              let snap = s.snapshots[id] else { throw TytoError.unknownDisplay("\(String(describing: displayID))") }
    ...
    func testClickToolSegment(_ index: Int) throws {
        guard session != nil else { throw TytoError.noSession }
