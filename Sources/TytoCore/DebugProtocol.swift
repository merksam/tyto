...
/// Newline-delimited JSON protocol between `tytoctl` and the running app's debug socket.
...
public enum DebugSocket {
    public static let bundleID = "com.yevhenii.tyto"

    /// `~/Library/Containers/com.yevhenii.tyto/Data`, built from the passwd home rather than
    /// `NSHomeDirectory()`: inside the sandbox `$HOME` already *is* this directory, while
    /// `tytoctl` runs unsandboxed. Computing it from the real home gives the same answer on
    /// both sides, so the app can bind the socket in its container and tytoctl can find it.
    public static var containerDataDirectory: String {
        realHomeDirectory() + "/Library/Containers/" + bundleID + "/Data"
    }

    /// Overridable with TYTO_DEBUG_SOCKET (must point inside the container for a sandboxed app
    /// to be able to bind it). ~70 bytes here; sockaddr_un allows 104.
    public static var path: String {
        if let p = ProcessInfo.processInfo.environment["TYTO_DEBUG_SOCKET"], !p.isEmpty { return p }
        return containerDataDirectory + "/tmp/tyto-debug.sock"
    }
