...
nohup "$APP/Contents/MacOS/Owl" >"$ROOT/out/owl.log" 2>&1 &
echo "launched pid $! (log: out/owl.log)" 1>&2

for i in $(seq 1 50); do
  if "$CTL" ping >/dev/null 2>&1; then
    "$CTL" ping
    echo "socket: $(ls -la "$("$CTL" container)/tmp/owl-debug.sock" 2>/dev/null || echo 'not in container (OWL_DEBUG_SOCKET override?)')" 1>&2
    exit 0
  fi
  sleep 0.1
done
